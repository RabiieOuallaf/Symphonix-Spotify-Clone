                                ...
                                <source src="{{Storage::url($music->music_audio)}}" id="audio" type="audio/mp3">
                                    ...
                                    <div class="flex items-center text-sm">
                                        <div class="relative w-28 h-28 mr-3 rounded-full md:block">
                                            <img class="object-cover w-full h-full rounded-full border-2 border-violet-800" id="musicImg" src="{{ Storage::url($music->music_banner) }}" alt="" loading="lazy" />
                                            <div class="absolute inset-0 rounded-full shadow-inner" aria-hidden="true"></div>
                                        </div>
                                        <div>
                                            <p class="font-semibold text-white" id="title">{{$music->song_name}}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-2 py-3 text-ms font-semibold border" id="artiste">{{$music->artist_name}}
                                
                                </td>
                                <td class="px-2 py-3 text-xs border">
                                    <span class="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-sm"> {{ $music->creating_date }} </span>
                                </td>
