        ...
        <div class="max-w-2xl w-full space-y-8 p-10 bg-neutral-900  rounded-xl shadow-lg z-10">
            <div class="grid  gap-8 grid-cols-1">
                <div class="flex flex-col ">
                    ...
                    <div class="mt-5">
                        <div class="form">
                            <form action="/music/update/{{$music->id}}" method="POST" enctype="multipart/form-data">
                                @csrf
                                ...
                                <div class="md:flex flex-row md:space-x-4 w-full text-xs">
                                    <div class="mb-3 space-y-2 w-full text-xs">
                                        ...
                                        <input placeholder="Music Name" class="appearance-none block w-full bg-grey-lighter text-grey-darker border border-grey-lighter rounded-lg h-10 px-4" required="required" type="text" name="song_name" value="{{$music->song_name}}">
                                        <p class="text-red text-xs hidden">Please fill out this field.</p>
                                    </div>
                                    <div class="mb-3 space-y-2 w-full text-xs">
                                        <label class="font-semibold text-white py-2">Lyrics writer <abbr title="required">*</abbr></label>
                                        <input placeholder="lyrics writer : " class="appearance-none block w-full bg-grey-lighter text-grey-darker border border-grey-lighter rounded-lg h-10 px-4" required="required" type="text" name="layrics_writer" value="{{$music->layrics_writer}}">
                                        <p class="text-red text-xs hidden">Please fill out this field.</p>
                                    </div>
                                </div>
                                <div class="mb-3 space-y-2 w-full text-xs">
                                    ...
                                    <div class="flex flex-wrap items-stretch w-full mb-4 relative">
                                        ...
                                        <input type="text" name="brand_artiste_website" class="flex-shrink flex-grow flex-auto leading-normal w-px flex-1 border border-l-0 h-10 border-grey-light rounded-lg rounded-l-none px-3 relative focus:border-blue focus:shadow" placeholder="https://" value="{{$music->brand_artiste_website}}">
                                    </div>
                                </div>
                                <div class="md:flex md:flex-row md:space-x-4 w-full text-xs">
                                    <div class="w-full flex flex-col mb-3">
                                        <label class="font-semibold text-white py-2">Music langauge</label>
                                        <input placeholder="musicLangauge" name="song_langauge" class="appearance-none block w-full bg-grey-lighter text-grey-darker border border-grey-lighter rounded-lg h-10 px-4" type="text" value="{{$music->song_langauge}}">
                                    </div>
                                    <div class="w-full flex flex-col mb-3">
                                        <label class="font-semibold text-white py-2">brand/artiste<abbr title="required">*</abbr></label>
                                        <select class="block w-full bg-grey-lighter text-grey-darker border border-grey-lighter rounded-lg h-10 px-4 md:w-full " required="required" value="{{$music->artist_name}}" name="artist_name">
